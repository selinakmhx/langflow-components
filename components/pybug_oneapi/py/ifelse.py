...
class ConditionalRouterComponent(Component):
    ...
    inputs = [
        MessageInput(
            name="input_text",
            display_name="Input",
            info="The primary message to route. Its text is used for comparison.",
            required=True,
        ),
        DropdownInput(
            name="operator",
        ...
        MessageInput(
            ...
            info="The message to pass if the condition is True. If not provided, the input message will be used.",
            advanced=True,
        ),
        MessageInput(
            name="false_case_message",
            display_name="Case False",
            info="The message to pass if the condition is False. If not provided, the input message will be used.",
            advanced=True,
        ),
        IntInput(
            name="max_iterations",
    ...
    def true_response(self) -> Message:
        result = self.evaluate_condition(
            self.input_text.text, self.match_text, self.operator, case_sensitive=self.case_sensitive
        )
        ...
        if result or force_output:
            output_message = self.input_text
            if self.true_case_message and self.true_case_message.text:
    ...
    def false_response(self) -> Message:
        ...
        if not result:
            output_message = self.input_text
            if self.false_case_message and self.false_case_message.text:
